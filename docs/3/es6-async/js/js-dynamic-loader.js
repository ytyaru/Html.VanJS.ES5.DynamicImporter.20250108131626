...
//const loadScript = (path, isDefer=false, isAsync=false, type='text/javascript') => {
const loadScript = (path, isAsync=false, type='text/javascript') => {
...
//            script.defer = isDefer;
            ...
            script.addEventListener('error', (ev) => {
                reject({
                    status: false, path:path,
                    message: `Failed to load the script ${path}`
                });
            });
...
//        this._state = 0 < this._options.paths.size ?  JsDynamicLoader.QueueState.Queueing: JsDynamicLoader.QueueState.Idling
    ...
    get isIdling() {return this._state===JsDynamicLoader.QueueState.Idling} // add()されるのを待っている（完了後もこれ）
    get isQueueing() {return this._state===JsDynamicLoader.QueueState.Queueing} // load()されるのを待っている
    get isLoading() {return this._state===JsDynamicLoader.QueueState.Processing} // onLoaded()されるのを待っている
...
//    get hasFailded() {return 0 < this._options.failedFullPaths.size}
    */
    #isProcessed(path) {
        ...
        if (this._options.loadedFullPaths.has(fullPath)) {console.error('************');return true}
        if (this._options.failedFullPaths.has(fullPath)) {console.error('VVVVVVVVVVVV');return true}
        return false
    }
    add(...paths) {
        // ロードに成功・失敗したパスは追加しない
        for (let p of [...paths].map(p=>this.#delHeadSlash(p)).filter(p=>!this.#isProcessed(p))) { this._options.paths.add(p) }
...
//        this._state = JsDynamicLoader.QueueState.Idling
    ...
    load(onLoaded, ...paths) { // 引数なしでもいい。constructor, addで設定できるから。
        ...
        this.add(...paths)
        console.log(this._options.paths)
        //if (!this.isQueueing) {console.warn(`一つ以上パスを追加してください。パスを追加する方法は三通りあります。
        if (0===this._options.paths.size) {console.warn(`一つ以上パスを追加してください。パスを追加する方法は三通りあります。
...
//        this._options.onLoaded([...this._options.loadedFullPaths], [...this._options.failedFullPaths])
//        this.clear()
        const loadedFullPaths = [...this._options.loadedFullPaths]
        const failedFullPaths = [...this._options.failedFullPaths]
        this.clear()
        this._options.onLoaded(loadedFullPaths, failedFullPaths)
        // results: [{isSucceeded:true, fullPath:'...'}, ...]
    }
    #createEl(path) {
        const script = document.createElement('script');
